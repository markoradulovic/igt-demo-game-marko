...
| Insufficient funds | `getResponseData` **throws** when `bet > balance`. `Game` gates Spin at the UI level and never calls the server in that state. Keeps `SpinResponse` a single clean type. |
...
| Promise rejection | On any rejection from `getResponseData`, `Game` transitions `REQUESTING → IDLE` and logs the error. No retry/toast in v1. |
...
1. **`src/server/slotMath.ts` (port + adapter)**
   - Port: `interface SlotServer { getResponseData(bet: number): Promise<SpinResponse> }`.
   - Adapter `MockedServer` implements it, hiding reel strips, paylines, paytable, PRNG, line evaluation, and wallet/balance state.
   - Types `SpinResponse`, `WinLine`, `Symbol` are colocated here — the only place outcome shape is defined. No standalone `types/` or `config/` directories.
   - Constructor accepts `{ seed?: number, startingBalance?: number }`. Defaults: random seed, `1000.00` balance.
...
- **`slotMath` / `MockedServer`** — yes. Suite covers:
  - Response-shape conformance (all required fields present, correct types).
  ...
  - Insufficient-funds: calling `getResponseData(bet)` when `bet > balance` throws a typed error; assert the throw and that balance is unchanged afterward.
